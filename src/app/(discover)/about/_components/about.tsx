...
const AboutGroup = ({ groupid, userid }: Props) => {
  ...
  // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  // Utility: Convert standard YouTube links (e.g. watch?v=, youtu.be) to embed format
  const getYoutubeEmbedUrl = (url: string) => {
    ...
    // Otherwise, assume it's already an embed URL or a valid link
    return url
  }
  // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            ...
            activeMedia?.type === "YOUTUBE" && (
              <div className="w-full aspect-video relative">
                <iframe
                  ...
                  // >>> Use the helper to get a proper embed URL <<<
